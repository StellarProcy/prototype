...
public class ViewController implements Renderable, ActionListener {
	...
	@Override
	public void render() {
		addIncome.setBounds(ViewUtil.consecutiveLocation(income, getInRect(),
				CATEGORY_RANGE, ViewUtil.LOCATE_DOWN));

		addExpense.setBounds(ViewUtil.consecutiveLocation(expense, getExRect(),
				CATEGORY_RANGE, ViewUtil.LOCATE_DOWN));

		addWallet.setBounds(ViewUtil.consecutiveLocation(wallets, getWaRect(),
				WALLET_RANGE, ViewUtil.LOCATE_RIGHT));

		frame.repaint();
	}

	// тут то и будут храниться все транзакции
	...
	private final Rectangle categoryRect = new Rectangle(10, 100, 140, 55);
	private final int CATEGORY_RANGE = 5;

	private final Rectangle walletRect = new Rectangle(10, 30, 130, 60);
	...
	private void createList(Collection<WalletImpl> src,
			List<ComponentManager> dest, Color color, Rectangle rect,
			ComponentOrientation orientation) {
		for (WalletImpl wi : src) {
			dest.add(create(wi, color, rect, orientation));
		}
		render();
	}
	...
	private void openDialog(ComponentManager cm) {
		...
		if (statement != ST_EDIT)
			...
			name = "Редактирование";
		dialog = new DialogFrame(frame, true, name + " "
				+ cm.getWallet().getName(), cm.getWallet().getAmount(),
				hasComment(cm));
	...
	private void addComponent(ComponentManager cm, List<ComponentManager> list,
			ComponentOrientation orientation) {
		ComponentManager clone = create(cm.getWallet().clone(),
				cm.button.getBackground(), cm.getBounds(), orientation);
		list.add(clone);
		clearBuffer();
		render();
	}
	...
	private void check(Object unknown, List<ComponentManager> list) {
		for (ComponentManager cm : list) {
			if (unknown.equals(cm.close)) {
				statement = ST_DEL;
				delBuffer = cm;
				confirmDialog = new ConfirmRemovingDialog(frame, true,
						cm.getName());
	...
	private void check(Object unknown, ConfirmRemovingDialog obj) {
		...
		if (unknown.equals(obj.cancelButton)) {
			...
			delete(frame.getContentPane(), delBuffer.getComponents());
	...
	private void newComponentDialog(int statement, Color color) {
		...
		buffer = create(new WalletImpl(-1, NO_NAME, new RubleUnit()), color,
				categoryRect, categoryOrientation);
		openDialog(buffer);
	}
